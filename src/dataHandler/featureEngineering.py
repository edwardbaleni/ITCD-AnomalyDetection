...
def radiusOfGyration(xx_centre, xx, yy):
    rad = 0
...
def shapeDescriptors(placeholder):
    ...
    placeholder["radius_of_gyration"] = placeholder[["centroid", "geometry"]].apply(lambda x: radiusOfGyration(x.iloc[0], x.iloc[1].exterior.coords.xy[0], x.iloc[1].exterior.coords.xy[1]), axis=1)
    # Robust Shape Descriptors
    placeholder[["minor_axis", "major_axis"]] = placeholder["geometry"].apply(lambda x: major_minor(x))
...
def zonalStatistics(placeholder, spectral):
    # Spectral Features
    geom = placeholder.loc[:,"geometry"]
    placeholder[["DEM_mean", "DEM_CV"]] = detStats(spectral["dem"], geom)
    placeholder[["NIR_mean", "NIR_CV"]] = detStats(spectral["nir"], geom)
    placeholder[["Red_mean", "Red_CV"]] = detStats(spectral["red"], geom)
    placeholder[["Reg_mean", "Reg_CV"]] = detStats(spectral["reg"], geom)
    # placeholder[["Green_mean", "Green_CV"]] = detStats(spectral["green"], geom)
    # placeholder[["Blue_mean", "Blue_CV"]] = detStats(spectral["blue"], geom)
    placeholder[["NDRE_mean", "NDRE_CV"]] = detStats(spectral["ndre"], geom)
    placeholder[["NDVI_mean", "NDVI_CV"]] = detStats(spectral["ndvi"], geom)
    placeholder[["GNVDI_mean", "GNVDI_CV"]] = detStats(spectral["gndvi"], geom)
    placeholder[["ENDVI_mean", "ENDVI_CV"]] = detStats(spectral["endvi"], geom)
    placeholder[["Intensity_mean", "Intensity_CV"]] = detStats(spectral["intensity"], geom)
    placeholder[["Saturation_mean", "Saturation_CV"]] = detStats(spectral["saturation"], geom)
    # placeholder[["DEM_max", "DEM_majority", "DEM_mean", "DEM_CV"]] = detStats(spectral["dem"], geom)
    # placeholder[["NIR_max", "NIR_majority", "NIR_mean", "NIR_CV"]] = detStats(spectral["nir"], geom)
    # placeholder[["Red_max", "Red_majority", "Red_mean", "Red_CV"]] = detStats(spectral["red"], geom)
    # placeholder[["Reg_max", "Reg_majority", "Reg_mean", "Reg_CV"]] = detStats(spectral["reg"], geom)
    # placeholder[["Green_max", "Green_majority", "Green_mean", "Green_CV"]] = detStats(spectral["green"], geom)
    # placeholder[["Blue_max", "Blue_majority", "Blue_mean", "Blue_CV"]] = detStats(spectral["blue"], geom)
    # placeholder[["NDRE_max", "NDRE_majority", "NDRE_mean", "NDRE_CV"]] = detStats(spectral["ndre"], geom)
    # placeholder[["NDVI_max", "NDVI_majority", "NDVI_mean", "NDVI_CV"]] = detStats(spectral["ndvi"], geom)
    # placeholder[["GNVDI_max", "GNVDI_majority", "GNVDI_mean", "GNVDI_CV"]] = detStats(spectral["gndvi"], geom)
    # placeholder[["ENDVI_max", "ENDVI_majority", "ENDVI_mean", "ENDVI_CV"]] = detStats(spectral["endvi"], geom)
    # placeholder[["Intensity_max", "Intensity_majority", "Intensity_mean", "Intensity_CV"]] = detStats(spectral["intensity"], geom)
    # placeholder[["Saturation_max", "Saturation_majority", "Saturation_mean", "Saturation_CV"]] = detStats(spectral["saturation"], geom)
    return placeholder

# TODO: https://iopscience.iop.org/article/10.1088/1361-6560/abfbf5/data
from sklearn.neighbors import NearestNeighbors as KNN
def distanceFeatures(placeholder):
...
def scaleData(placeholder, spectral):
    ...
    placeholder = shapeDescriptors(placeholder)
    placeholder = distanceFeatures(placeholder)
    # zonal statistics have to come last as crs is 4326 
    # and above crs is converted to 3857 to work with the shape descriptors
    # start = timer()
    placeholder = zonalStatistics(placeholder, spectralData)
    # end = timer()
    # print(end - start)
...
if __name__ == "__main__":
    ...
    # os.chdir("..")
    sampleSize = 20
    data_paths_tif, data_paths_geojson, data_paths_geojson_zipped = dataCollect.collectFiles(sampleSize)

    num = 0
    # start = timer()
    spectralData, delineations, mask, ref_data = dataCollect.fixData(data_paths_tif[num], data_paths_geojson[num], data_paths_geojson_zipped[num])
    spectralData = dataCollect.vegIndices(spectralData)
    # end = timer()
    # print(end - start)

    data = scaleData(delineations, spectralData)
