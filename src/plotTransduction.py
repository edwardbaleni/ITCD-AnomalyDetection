...
PR1, AP_1 = PrecisionRecall(P_1, R_1, AP_1, "Orchard-1")
PR2, AP_2 = PrecisionRecall(P_2, R_2, AP_2,"Orchard-2")
PR3, AP_3 = PrecisionRecall(P_3, R_3, AP_3,"Orchard-3")
PR4, AP_4 = PrecisionRecall(P_4, R_4, AP_4,"Orchard-4")
PR5, AP_5 = PrecisionRecall(P_5, R_5, AP_5,"Orchard-5")

PR = pd.concat([PR1, PR2, PR3, PR4, PR5], axis=0)
PR.reset_index(drop=True, inplace=True)

AP = pd.concat([AP_1, AP_2, AP_3, AP_4, AP_5], axis=0)
AP.reset_index(drop=True, inplace=True)
...
# Add Type column to PR and AP dataframes
PR['Type'] = PR['Estimator'].map(lambda x: type_mapping.get(x, 'Density'))
AP['Type'] = AP['Estimator'].map(lambda x: type_mapping.get(x, 'Density'))
...
for ax in g.axes.flat:
    ...
    for _, row in AP.iterrows():
        # if estimator and orchard exist in axis then add the auc value
        if row['Estimator'] in PR[PR['Type'] == learn_approach]['Estimator'].unique() and row['Orchard'] == orchard_number:
            color = palette[PR['Estimator'].unique().tolist().index(row['Estimator'])]
            ax.text(0.6, y_pos, f"AP: {row['AP']}", transform=ax.transAxes, fontsize=20, fontweight='bold', verticalalignment='top', color=color)
            y_pos += 0.05
